      ...
      <main className="max-w-5xl mx-auto p-6">
        {tab === '📅 Уроки' && (
          <div className="space-y-6"><h2 className="text-lg font-semibold text-gray-800">Уроки {student.name}</h2>
            <div className="grid md:grid-cols-2 gap-6"><div className="glass-card rounded-2xl p-4"><Calendar onChange={(date) => setSelectedDate(date as Date)} value={selectedDate} tileClassName={git add .({ date }) => { const d = date.toISOString().split('T')[0]; return lessons.some((l) => l.date === d) ? 'has-lesson' : '' }} /></div>
